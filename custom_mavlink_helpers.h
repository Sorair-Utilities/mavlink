...
// ───────────────────────────────────────────────────────────────────────────
// uAvionix CRC override: make table mutable and patch IDs 201/202/203
// Must be at file-scope so constructor runs.
// ───────────────────────────────────────────────────────────────────────────
#if MAVLINK_CRC_EXTRA
#undef MAVLINK_MESSAGE_CRC
static uint8_t mavlink_message_crcs[256] = MAVLINK_MESSAGE_CRCS;
#define MAVLINK_MESSAGE_CRC(msgid)                                   \
    (                                                              \
      (msgid) == 201 ? 126 /* STATIC */                            \
    : (msgid) == 202 ?   7 /* DYNAMIC */                           \
    : (msgid) == 203 ?  85 /* STATUS  */                           \
    : mavlink_message_crcs[msgid]                                  \
    )

__attribute__((constructor))
static void _uavionix_crc_patch(void) {
    mavlink_message_crcs[201] = 126;
    mavlink_message_crcs[202] =   7;
    mavlink_message_crcs[203] =  85;
}
#endif
// ───────────────────────────────────────────────────────────────────────────
